...
import type { Response, Result } from '~/types/response'
...
onMounted(() => {
  // 设置主题
  chatStore.setSubjectId(chatId.value)
  if (chatStore.getParentMessageId !== '0') {
    // 获取当前用户聊天主题记录
    chatStore.setMessageLoading(true)
    const http = new Http()
    http.get(`/lin_chat/message/messages/${chatId.value}`).then((res) => {
      const chatRecord = res as Response<Result<ChatMessage>>
      chatStore.setChatRecord(chatRecord.result.list)
...
<template>
  ...
  >
    <ul>
      <div v-for="(item, index) in chatStore.getChatMessage" :key="index" class="px-3 py-2">
        <!-- 问 -->
        ...
        <!-- 答 -->
        <div v-show="(index !== getChatRecord.length - 1) || !errorMessage" text-left>
          ...
          <li class="inline-block max-w-8/10 w-max rounded-lg dark:bg-dark">
            <MdPreview :editor-id="id" :model-value="item.answer.content" :theme="color.value === 'dark' ? 'dark' : 'light'" class="border-rd-2" />
          </li>
            ...
            <el-dropdown class="custom-align">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  ...
                  <el-dropdown-item :icon="ElIconCopyDocument" @click="copy(item.answer.content)">
                    复制
                  </el-dropdown-item>
                </el-dropdown-menu>
              </template>
...
:deep().md-editor-preview-wrapper {
  ...
  /* color: #FFF !important; */
  @apply dark:bg-dark bg-white !text-dark !dark:text-light-500
}
/* :deep().md-editor-preview-wrapper .dark{
  color:#FFFFFF !important;
} */
:deep().default-theme{
  @apply !dark:text-light-500 text-dark
}
.custom-align {
